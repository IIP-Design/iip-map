      ...
      if (card.time.toggled === true) {
        if (card.time.timeFormat === "24hour") {
          const time = convertTime12to24(
            `${field[0].hours}:${field[0].minutes} ${field[0].am_pm}`
          );
          return `${time}`;
        }
...
class Table extends Component {
  ...
  render() {
    const { isLoading, events, error, filter, checked } = this.state;
            ...
            filteredData.map(event => {
              ...
              return (
                ...
                >
                  ...
                  <div className="location">{event.properties.location}</div>
                  <div className="contact">
                    {buildAdditional(additionalData)
                      .split("\n")
                      .map(row => (
                        <div key={Math.random() * 1000}>{row}</div>
                      ))}
                  </div>
                </div>
              );
